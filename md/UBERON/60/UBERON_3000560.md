
## Class : septum semicircularium anterior

 * *IRI* = http://purl.obolibrary.org/obo/UBERON_3000560
 * *Label* = septum semicircularium anterior
 * *Definition* = The anterior connective tissue partition between walls of otic capsule around which the membranous semicircular canals pass.

## Synonyms


## Cross-references


## Subsets


## Superclasses

 * [septum](../../UBERON/37/UBERON_0003037.md)
 * [ectoderm-derived structure](../../UBERON/21/UBERON_0004121.md)
 * [structure with developmental contribution from neural crest](../../UBERON/14/UBERON_0010314.md)
 * [part_of](../../BFO/50/BFO_0000050.md) some [internal ear](../../UBERON/46/UBERON_0001846.md)

## Equivalencies


## Other Logical Axioms


## Other Annotations

 * *[definition](../../IAO/15/IAO_0000115.md)* = The anterior connective tissue partition between walls of otic capsule around which the membranous semicircular canals pass.
 * *[core#provenance_notes](../../core#provenance/es/core#provenance_notes.md)* = This class was sourced from an external ontology (amphibian_anatomy). Its definitions, naming conventions and relationships may need to be checked for compatibility with uberon
 * *[oboInOwl#created_by](../../oboInOwl#created/by/oboInOwl#created_by.md)* = amphibian_anatomy_curators
 * *[rdf-schema#label](../../el/rdf-schema#label.md)* = septum semicircularium anterior

## Usage

 * -
 * [septum semicircularium anterior](../../UBERON/60/UBERON_3000560.md) SubClassOf [ectoderm-derived structure](../../UBERON/21/UBERON_0004121.md)
 * [septum semicircularium anterior](../../UBERON/60/UBERON_3000560.md) SubClassOf [part_of](../../BFO/50/BFO_0000050.md) some [internal ear](../../UBERON/46/UBERON_0001846.md)
 * [septum semicircularium anterior](../../UBERON/60/UBERON_3000560.md) SubClassOf [septum](../../UBERON/37/UBERON_0003037.md)
 * [septum semicircularium anterior](../../UBERON/60/UBERON_3000560.md) SubClassOf [structure with developmental contribution from neural crest](../../UBERON/14/UBERON_0010314.md)

## External Comments

